    ...
    return room && (
        <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
                ...
                <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
                    {room?.images.length > 1 && room.images.map((image, index) => (
                        <img onClick={() => setMainImage(image)}
                            ...
                            className={`w-full rounded-xl shadow-md object-cover cursor-pointer h-44 ${mainImage === image && 'outline outline-orange-500'}`} />
                    ))}
                </div>
            </div>
            ...
            <div className='flex flex-col md:flex-row md:justify-between mt-10 gap-6'>
                ...
                {/* Room Price  */}
                <p className='text-2xl font-medium text-gray-800 shrink-0'>${room.pricePerNight}<span className='text-sm font-light text-gray-500'> / night</span></p>
            </div>
